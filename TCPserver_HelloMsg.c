/*	======================================================================
	...
	D. accept()
		...
		*	Synopsis: int accept(int sockfd, struct sockaddr *addr, 
										socklen_t *addrlen);
	...
	E. send()
		*	Task:	Send Data
		*	Synopsis: ssize_t send(int sockfd, const void *buf, 
	...
	F. recv()
		*	Task:	Receive Data
		*	Synopsis: ssize_t recv(int sockfd, void *buf, 
										size_t len, int flags);
	...
	F.	Byte Order (BO) decides how a number is stored and sent.
		*	There are two types of BO:
			1. Big Endian:	High-order byte of the number is stored in 
						 		memory at the lowest address and sent at first.
			2. Little Endian:	High-order byte of the number is stored in  
									memory at the lowest address and sent at 
									first.

		*	NBO (Network Byte Order): 	decides how data will be exchanged 
												between hosts.
			HBO (Host Byte Order):	decides how data will be stored inside
											a specific host.
